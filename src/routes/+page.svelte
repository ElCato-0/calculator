<script>
    import SquareRootIcon from "$lib/icons/SquareRoot.svelte";
    import MinusIcon from "$lib/icons/Minus.svelte";
    import DivisionIcon from "$lib/icons/Division.svelte";
    import MultiplyIcon from "$lib/icons/Multiply.svelte";
    import PlusIcon from "$lib/icons/Plus.svelte";
</script>

<svelte:head>
    <title>الة حاسبة</title>
</svelte:head>

<div
    class="bg-white rounded-3xl grid grid-cols-4 gap-1 p-6 font-semibold text-xl shadow-xl"
>
    <div
        class="bg-blue-500 rounded-full col-span-4 h-12 flex items-center px-4 mb-2 text-white"
    >
        123456
    </div>
    <button class="bg-[#f3f6fc]"> % </button>
    <button class="bg-[#f3f6fc]">
        <SquareRootIcon />
    </button>
    <button class="bg-[#f3f6fc]"> CE </button>
    <button class="bg-[#1f2b54] text-white"> C </button>
    <button> 7 </button>
    <button> 8 </button>
    <button> 9 </button>
    <button class="bg-[#fd3f59] text-white">
        <MinusIcon />
    </button>
    <button> 4 </button>
    <button> 5 </button>
    <button> 6 </button>
    <button class="bg-[#2285fd] text-white">
        <DivisionIcon />
    </button>
    <button> 1 </button>
    <button> 2 </button>
    <button> 3 </button>
    <button class="bg-[#f9c80e] text-white">
        <MultiplyIcon />
    </button>
    <button> . </button>
    <button> 0 </button>
    <button> = </button>
    <button class="bg-[#63dc74] text-white">
        <PlusIcon />
    </button>
</div>
